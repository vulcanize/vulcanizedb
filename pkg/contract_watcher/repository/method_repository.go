...
func (r *methodRepository) persistResults(results []types.Result, methodInfo types.Method, contractAddr, contractName string) error {
	...
	for _, result := range results {
		...
		pgStr := fmt.Sprintf("INSERT INTO cw_%s.%s_method ", strings.ToLower(contractAddr), strings.ToLower(result.Name))
...
func (r *methodRepository) CreateMethodTable(contractAddr string, method types.Method) (bool, error) {
	tableID := fmt.Sprintf("cw_%s.%s_method", strings.ToLower(contractAddr), strings.ToLower(method.Name))
...
func (r *methodRepository) checkForTable(contractAddr string, methodName string) (bool, error) {
	pgStr := fmt.Sprintf("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = '%s' AND table_name = '%s_method')", strings.ToLower(contractAddr), strings.ToLower(methodName))
	var exists bool

...
var _ = Describe("Poller", func() {
	...
	BeforeEach(func() {
		...
		contractPoller = poller.NewPoller(bc, db)
	})
	...
	Describe("PollContract", func() {
		It("Polls specified contract methods using contract's token holder address list", func() {
			...
			err = db.QueryRowx(fmt.Sprintf("SELECT * FROM cw_%s.balanceof_method WHERE who_ = '0xfE9e8709d3215310075d67E3ed32A380CCf451C8' AND block = '6707322'", constants.TusdContractAddress)).StructScan(&scanStruct)
			Expect(err).ToNot(HaveOccurred())
			Expect(scanStruct.Balance).To(Equal("66386309548896882859581786"))
			Expect(scanStruct.TokenName).To(Equal("TrueUSD"))

			err = db.QueryRowx(fmt.Sprintf("SELECT * FROM cw_%s.balanceof_method WHERE who_ = '0xfE9e8709d3215310075d67E3ed32A380CCf451C8' AND block = '6707323'", constants.TusdContractAddress)).StructScan(&scanStruct)
			Expect(err).ToNot(HaveOccurred())
			Expect(scanStruct.Balance).To(Equal("66386309548896882859581786"))
			Expect(scanStruct.TokenName).To(Equal("TrueUSD"))

			err = db.QueryRowx(fmt.Sprintf("SELECT * FROM cw_%s.balanceof_method WHERE who_ = '0x3f5CE5FBFe3E9af3971dD833D26bA9b5C936f0bE' AND block = '6707322'", constants.TusdContractAddress)).StructScan(&scanStruct)
			Expect(err).ToNot(HaveOccurred())
			Expect(scanStruct.Balance).To(Equal("17982350181394112023885864"))
			Expect(scanStruct.TokenName).To(Equal("TrueUSD"))

			err = db.QueryRowx(fmt.Sprintf("SELECT * FROM cw_%s.balanceof_method WHERE who_ = '0x3f5CE5FBFe3E9af3971dD833D26bA9b5C936f0bE' AND block = '6707323'", constants.TusdContractAddress)).StructScan(&scanStruct)
			Expect(err).ToNot(HaveOccurred())
			Expect(scanStruct.Balance).To(Equal("17982350181394112023885864"))
			Expect(scanStruct.TokenName).To(Equal("TrueUSD"))
		})
		...
		It("Polls specified contract methods using contract's hash list", func() {
			...
			err = db.QueryRowx(fmt.Sprintf("SELECT * FROM cw_%s.owner_method WHERE node_ = '0x7e74a86b6e146964fb965db04dc2590516da77f720bb6759337bf5632415fd86' AND block = '6885877'", constants.EnsContractAddress)).StructScan(&scanStruct)
			Expect(err).ToNot(HaveOccurred())
			...
			err = db.QueryRowx(fmt.Sprintf("SELECT * FROM cw_%s.owner_method WHERE node_ = '0x93cdeb708b7545dc668eb9280176169d1c33cfd8ed6f04690a0bcc88a93fc4ae' AND block = '6885877'", constants.EnsContractAddress)).StructScan(&scanStruct)
			Expect(err).ToNot(HaveOccurred())
		...
		It("Caches returned values of the appropriate types for downstream method polling if method piping is turned on", func() {
			...
			err = db.QueryRowx(fmt.Sprintf("SELECT * FROM cw_%s.resolver_method WHERE node_ = '0x495b6e6efdedb750aa519919b5cf282bdaa86067b82a2293a3ff5723527141e8' AND block = '6921967'", constants.EnsContractAddress)).StructScan(&scanStruct)
			Expect(err).ToNot(HaveOccurred())

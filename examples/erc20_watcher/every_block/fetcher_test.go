...
var _ = Describe("ERC20 Fetcher", func() {
	...
	Describe("FetchBalanceOf", func() {
		It("fetches data from the blockchain with the correct arguments", func() {
			_, err := testFetcher.FetchBalanceOf(testAbi, testContractAddress, "testOwnerAddress", blockNumber)
			...
			Expect(fakeBlockchain.FetchedMethod).To(Equal("balanceOf"))
			Expect(fakeBlockchain.FetchedMethodArg).To(BeZero())
		...
		It("fetches the balance for a given block and owner from a real blockchain", func() {
			daiOwner := "0x4a16ecf42d72528264b8313b604493eafef5d845"

			result, err := realFetcher.FetchBalanceOf(constants.DaiAbiString, constants.DaiContractAddress, daiOwner, blockNumber)
			expectedResult := big.Int{}
			expectedResult.SetString("4611129918126858270555334", 10)
			Expect(err).NotTo(HaveOccurred())
			Expect(result).To(Equal(expectedResult))
		})

		It("returns an error if the call to the blockchain fails", func() {
			_, err := errorFetcher.FetchBalanceOf("", "", "", 0)

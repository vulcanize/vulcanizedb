...
var _ = Describe("Storage value metadata getter", func() {
	It("returns storage value metadata for a single storage variable", func() {
		...
		expectedMetadata := storage.StorageValueMetadata{
			Name: metadataName,
			Keys: metadataKeys,
			Type: metadataType,
		}
		Expect(storage.GetStorageValueMetadata(metadataName, metadataKeys, metadataType)).To(Equal(expectedMetadata))
	})
	...
	Describe("metadata for a packed storaged slot", func() {
		It("returns metadata for multiple storage variables", func() {
			...
			Expect(storage.GetStorageValueMetadataForPackedSlot(metadataName, metadataKeys, metadataType, metadataPackedNames, metadataPackedTypes)).To(Equal(expectedMetadata))
		})
		...
		It("panics if PackedTypes are nil when the type is PackedSlot", func() {
			...
			getMetadata := func() {
				storage.GetStorageValueMetadataForPackedSlot(metadataName, metadataKeys, metadataType, metadataPackedNames, nil)
			}
		...
		It("panics if PackedNames are nil when the type is PackedSlot", func() {
			...
			getMetadata := func() {
				storage.GetStorageValueMetadataForPackedSlot(metadataName, metadataKeys, metadataType, nil, metadataPackedTypes)
			}
			Expect(getMetadata).To(Panic())
		})
		...
		It("panics if valueType is not PackedSlot if PackedNames is populated", func() {
			...
			getMetadata := func() {
				storage.GetStorageValueMetadataForPackedSlot(metadataName, metadataKeys, metadataType, metadataPackedNames, metadataPackedTypes)
			}
			Expect(getMetadata).To(Panic())
		})
	})
